...
object Dictionaries {
    ...
    val MONEY = listOf(
        ...
        // Голое «начислен» сюда не годится: под него попадают «вам начислено
        // 500 бонусов» и прочие бонусные рассылки, а рекламный словарь проверяется
        // позже банковского. Тот же принцип, по которому из этого списка убраны
        // символы валют. Поэтому только проценты по вкладу — тот случай,
        // который не ловился вообще ничем.
    ...
    // Собирается один раз. Раньше PROMO_RU + PROMO_EN создавал новый список
    // из трёхсот с лишним строк на каждый вызов — а вызывается это и из
    // Prefs.snapshot() на каждое уведомление, и из композиции через promoSize().
