...
/**
 * Регион словаря. Влияет только на рекламные слова.
 *
 * Защитные списки — экстренные оповещения, состояние устройства, коды,
 * операции по счёту, статусы доставки — общие для всех регионов и всегда
 * проверяются целиком: код подтверждения может прийти на любом языке,
 * и ошибиться здесь дороже, чем пропустить рекламу.
 */
enum class Region { RU, EN, ALL }

object Dictionaries {

    /* --------------------- защитные списки: всегда общие -------------------- */
    ...
    /**
     * Английский словарь. Немецкие выражения включены сюда же:
     * латиница внутри одного словаря не даёт ложных срабатываний
     * на английском тексте, а Германия — крупная аудитория приложения.
     */
    val PROMO_EN = listOf(
        // покупки
