...
/**
 * Причина из журнала в текст на языке интерфейса.
 *
 * Держим отдельно от Strings: тащить два десятка строк в и без того здоровенный
 * конструктор незачем. Старые записи (там лежал готовый русский текст) узнаются
 * по отсутствию кода и показываются как есть, миграция журнала не нужна.
 */
object ReasonText {
    ...
    fun render(stored: String, lang: Lang): String {
        ...
        val code = runCatching { ReasonCode.valueOf(codeName) }.getOrNull()
            ?: return stored // запись старого формата
