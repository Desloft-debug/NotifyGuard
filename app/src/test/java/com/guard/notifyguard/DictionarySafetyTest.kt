...
/**
 * Главный смысловой тест проекта: важное сообщение не должно попадать под рекламный словарь.
 *
 * Проверяются два разных свойства.
 *
 * 1. Контрольный текст ловится защитным словарём (EMERGENCY / SYSTEM / DELIVERY /
 *    CODE / MONEY). Они проверяются раньше промо, поэтому попадание в них — гарантия,
 *    что уведомление дойдёт до человека даже в строгом режиме.
 * 2. Ни одно рекламное слово на контрольный текст не срабатывает. Это подстраховка
 *    на случай, если порядок проверок когда-нибудь поменяют.
 *
 * Словари правятся чаще всего остального, а последствия ошибки — потерянный код
 * из банка. Тест должен падать раньше, чем это увидит пользователь.
 */
class DictionarySafetyTest {
    ...
    fun `бонусные баллы не считаются операцией по счёту`() {
        // «начислен» в списке денег стоит только в связке с процентами.
        // Голое «начислен» пропускало бы бонусные рассылки как банковские операции:
        // денежный словарь проверяется раньше рекламного.
    ...
    fun `промокод не считается кодом подтверждения`() {
        // Ради этого правила и появились границы слов.
    ...
    fun `рекламное слово не перекрывает защитное`() {
        // Если промо-слово оказывается началом защитного (или наоборот), словари
        // начинают спорить друг с другом, и разрешение спора зависит от порядка проверок.
